...
class GymExperiment:
    def __init__(self, env_config: dict, alg_config: dict, exp_config: dict,
                 run_one_episode, create_agent, save_experiment_output):
        ...
        self.n_episodes = exp_config.get("n_episodes", 100)

        self.run_one_episode = run_one_episode
    ...
    def run(self):
        ...
        for i in range(self.exp_repeat):
            ...
            self.save_experiment_output(i, output, exp_subfolder)
    ...
    def run_one_experiment(self, env, alg_config):
        exp_train_output = []
        agent = self.create_agent(alg_config)
        for i in range(self.n_episodes):
            episode_output = self.run_one_episode(env, agent)
